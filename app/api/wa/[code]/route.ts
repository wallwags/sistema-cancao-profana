...
export async function GET(req: NextRequest, { params }: { params: { code: string } }) {
  ...
  const link = `${origin}/?b=${code}`;
  const msg = `Convite do Concurso Cancao Profana! ${leader} inscreveu ${bandName} e voce foi escalado como integrante. Confirme seus dados e garanta sua parte aqui: ${link}`;
