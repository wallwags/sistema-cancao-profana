...
import { Loader2, Music, Users, Shield, Check } from 'lucide-react';
...
export default function Page() {
  ...
  const handleOpenQuiz = () => {
    // Aberta tambem durante a live: nesse caso o servidor registra com preco exclusivo da Live
    setQuizMounted(true);
    setIsQuizOpen(true);
  };
      ...
      <main className="max-w-6xl mx-auto px-6 pt-32 sm:pt-40 pb-10 grow space-y-24 relative z-10">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 items-center">
          <div className="lg:col-span-7 space-y-4">
            ...
            <div className="fade-up-800 [animation-delay:260ms] flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-3">
              ...
              >
                {isLiveNow ? 'INSCREVER-SE · AO VIVO' : 'INSCREVER-SE'}
              </button>
              <a
                href="#premios"
        ...
        <section id="lotes" className="space-y-12">
          ...
          <div className="pt-4 text-center">
            ...
            >
              {isLiveNow ? 'Garantir Inscrição ao Vivo' : 'Garantir Inscrição Lote 1'}
            </button>
          </div>
        </section>
            ...
            {!waitlistDone ? (
              <>
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="email" value={waitlistEmail} onChange={(e) => { setWaitlistEmail(e.target.value); if (waitlistError) setWaitlistError(''); }} onKeyDown={(e) => { if (e.key === 'Enter' && !waitlistBusy && waitlistEmail.trim()) { if (vipWaUrl) vipAnchorRef.current?.click(); else handleVipClick(); }; }} placeholder="voce@email.com" className="w-full bg-black/60 border border-white/10 rounded-xl px-4 py-3.5 text-white text-base outline-none focus:border-[#E3B552] placeholder-gray-600" autoComplete="email" autoFocus />
                </div>
                {waitlistError && (
