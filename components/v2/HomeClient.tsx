...
const QuizFlow = dynamic(() => import('./QuizFlow'), { ssr: false });
...
// Configuracao hidratada pelo SERVIDOR (app/page.tsx busca no Supabase antes de servir o HTML:
// a home chega pronta e correta na primeira visita, sem flash e sem depender da rede do visitante)
export interface HomeCfg {
  lotes?: LotesConfig;
  loteDates?: Record<string, string | null>;
  liveStatusBar?: 'em_breve' | 'ao_vivo' | 'encerrada';
  countdownTarget?: string | null;
  liveLaunch?: string | null;
  cartOpen?: string | null;
  liveUrl?: string | null;
  dia0Price?: number;
  slotMode?: 'band' | 'integrante';
...
export default function HomeClient({ cfg }: { cfg: HomeCfg | null }) {
  const [lotesConfig, setLotesConfig] = useState<LotesConfig>(cfg?.lotes ?? {
  ...
  const [faqList, setFaqList] = useState<Array<{ q: string; a: string }>>(cfg?.faq && cfg.faq.length > 0 ? cfg.faq : [
  ...
  const [countdownTarget, setCountdownTarget] = useState<string | null>(cfg?.countdownTarget ?? null);
  const [liveLaunch, setLiveLaunch] = useState<string | null>(cfg?.liveLaunch ?? null);
  const [cartOpen, setCartOpen] = useState<string | null>(cfg?.cartOpen ?? null); // abertura real das inscricoes (fonte da faixa pre-live)
  const [cupom, setCupom] = useState<string | null>(null); // header/CTA so aparecem quando a config real chegou (ou snapshot do cache) // cupom de lote (via ?cupom=) // abertura real das inscricoes (fonte da faixa pre-live)
  const [liveUrl, setLiveUrl] = useState<string | null>(cfg?.liveUrl ?? null);
  const [dia0Price, setDia0Price] = useState<number>(cfg?.dia0Price ?? 25);
  const [liveStatusBar, setLiveStatusBar] = useState<'em_breve' | 'ao_vivo' | 'encerrada'>(cfg?.liveStatusBar ?? 'em_breve');
  const [waitlistOpen, setWaitlistOpen] = useState(false);
  const [vipWaUrl, setVipWaUrl] = useState<string>(cfg?.vipWaUrl ?? '');
  ...
  const [waitlistMode, setWaitlistMode] = useState<boolean>(cfg?.homeCtaMode !== 'quiz'); // pré-inscrição é o estado padrão
  const [slotMode, setSlotMode] = useState<'band' | 'integrante'>(cfg?.slotMode ?? 'band');
  const [sheetCode, setSheetCode] = useState<string | null>(null);
  const [sheetStart, setSheetStart] = useState<'confirm' | 'pick'>('confirm');
  const [loteDates, setLoteDates] = useState<Record<string, string | null>>(cfg?.loteDates ?? {});
          ...
          <div className="legal-pop w-full max-w-md bg-[#05070B] border-2 border-[#E3B552] rounded-[28px] p-6 sm:p-8 space-y-5 shadow-[0_10px_60px_rgba(0,0,0,0.9)] relative" onClick={(e) => e.stopPropagation()}>
            ...
            {!waitlistDone ? (
              <>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter' && !waitlistBusy && waitlistEmail.trim()) { if (vipWaUrl) vipAnchorRef.current?.click(); else handleVipClick(); }; }}
                ...
                {vipWaUrl ? (
                  <a
                    ...
                    className="w-full flex items-center justify-center gap-2 font-display font-bold text-sm uppercase tracking-wider text-black bg-gradient-to-b from-[#34D399] to-[#059669] py-3.5 rounded-full shadow-lg shadow-[#10B981]/25 active:scale-[0.98] transition-transform"
                  >
                    {waitlistBusy ? <Loader2 className="w-5 h-5 animate-spin" /> : <Users className="w-4 h-4" />}
                    {waitlistBusy ? 'Registrando...' : 'ENTRAR NO GRUPO VIP'}
                  </a>
                ) : (
                  <button
                    onClick={handleVipClick}
              ...
              <div className="text-center space-y-3 py-4">
                ...
                <p className="text-sm text-gray-300 leading-relaxed max-w-xs mx-auto">Agora é só finalizar a entrada no WhatsApp. Se a aba não abriu, verifique o bloqueador de popups do navegador.</p>
              </div>
            )}
          </div>
        </div>
